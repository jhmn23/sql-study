...
-- 그룹함수에서는 null값을 제외하고 계산한다.(예외 COUNT(*))
...
FROM employees; -- 널 값을 제외한 모든 직원들의 커미션 평균값
...
-- 예제 1)
SELECT department_id 부서번호, COUNT(*) 사원수, MAX(salary) 최대급여, MIN(salary) 최소급여,
...
-- 예제 3)
SELECT job_id 직업번호, SUM(salary) 급여총액
FROM employees
