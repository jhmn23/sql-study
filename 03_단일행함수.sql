...
-- 예제 2)
SELECT last_name 이름, TO_CHAR( salary, '$99,999.00') 월급 
FROM employees
WHERE salary > 10000
...
-- NVL2(값 , 첫번째 , 두번째) => 만약 값이 null값이 아니면 첫번째 , null값이면 두번째
...
-- decode 함수 DECODE(a, b, c) a의 값이 b면 c로 출력
SELECT last_name 이름, job_id, salary,
    DECODE(job_id, 'IT_PORG', salary * 1.10,
